#!/usr/bin/env python3
"""Second external RUP checker plugin for quorum conformance.

This intentionally shares the same RUP semantics as `rup_plugin.py` but lives at
a distinct entrypoint hash, so quorum fixtures can exercise the runner's
hash-based independence guard without adding a new runtime dependency.
"""
import argparse
import json
import sys
...
def emit(verdict: str, detail: str, rung: str = "R3") -> int:
    print(json.dumps({
        ...
        "detail": detail,
        "rung": rung,
        "checker": {"name": "barrier-atlas-rup-alt", "version": "0.1"},
    }, sort_keys=True))
    return 0
