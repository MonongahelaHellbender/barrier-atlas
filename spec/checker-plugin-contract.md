...
the plugin should read artifacts only from `--artifacts-dir` by id.
...
timeout, invalid JSON, illegal verdict, or illegal rung becomes
`UNVERIFIABLE-HERE`, never `CERTIFIED`.
...
binding, and rung ceilings, not sandboxing or proof of plugin honesty.
